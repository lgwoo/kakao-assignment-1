...
function getEmptyMessage(filter: string): string {
  if (filter === 'active') return '진행 중인 할 일이 없어요.'
  if (filter === 'completed') return '완료된 할 일이 없어요.'
  return '이 날의 할 일이 없어요. 추가해보세요!'
}

export default function TodoList({ todos, filter, selectedDate, onToggleComplete, onDelete, onSave }: TodoListProps) {
